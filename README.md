...
核心設計的靈感與 skill 流程觀念，汲取自同事 toomore 於 OCF 六月內部知識分享的 agents writing pipeline 簡報：先找題與備料，再由主 session 起草，最後用平行審稿與查核收尾。本 repo 不保存原始簡報檔；這裡把那套流程觀念改寫成 Ian Open News 的 GitHub-native 工作流，讓新增資料、分類、審核狀態與摘要都能透過 Issue / PR / GitHub Actions 管理。

## Repo 結構

- `reference/`：原始資料匯入參考檔，不直接編輯；不保存同事內部分享簡報原檔。
...
- `.claude/`：依 toomore 內部分享啟發後，整理成本專案使用的 agent 與 slash command 範本。
- `CLAUDE.md`：常駐寫作規則，每次 session 自動載入。
- `scripts/`：匯入、抓 RSS、驗證、匯出 SQLite、本機網頁工具。

## Agents 寫作生產線

把每次發文都會漏的「寫作風格、查證、觀點」交給固定角色顧。流程靈感來自 toomore 的 OCF 內部知識分享簡報；repo 內的操作版與致謝說明見 **[docs/agents-pipeline.md](docs/agents-pipeline.md)**，整體工作流見 [docs/workflow.md](docs/workflow.md)。
...
從關鍵字篩選一路到開 PR 的完整流程圖見 [docs/agents-pipeline.md](docs/agents-pipeline.md#整體流程篩完關鍵字--開-pr)，日常工作流見 [docs/workflow.md](docs/workflow.md)。
...
如果是從本機網頁首頁手動按「抓到 RSS 待整理」，會呼叫 `python3 scripts/local_rss_daily.py --manual`，額外包含設定為「按更新時抓」的來源。
...
4. 依 `fetch_frequency` 判斷是否到期：每 1 小時、每 6 小時、每天、每週、每月；「按更新時抓」只在首頁手動更新或單一來源手動更新時處理；「暫停抓取」不抓。
5. 把新項目新增到 `.cache/rss-candidates.jsonl`；一般來源抓近 7 天，第一次成功抓取前的新來源會先回補近 90 天。
6. 用 `database/triage-keywords.json` 標示「建議收」或「建議不要看」。
7. 用 Codex CLI 補閱讀建議與摘要。
8. 你在本機網頁「RSS 待整理」按確認收或直接送 PR 後，才會寫進 `database/items.jsonl` 並套用決定。

要指定/停止來源，直接改 `database/sources.jsonl` 或用本機網頁：

- 要抓：`status` 設為 `active`，`track` 設為兩條主線之一。
- 控制節奏：`fetch_frequency` 可設 `hourly`、`six-hourly`、`daily`、`weekly`、`monthly`、`on-update` 或 `paused`。
- 暫停整個來源：`status` 設為 `paused`。
