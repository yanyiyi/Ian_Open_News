...
表單底部可以直接選「存好之後怎麼分流」，套用和入庫建檔區同一套三篩選決定（含相同的 `local_decision` 與 review 事件）：

- 確認收，直接放入可用材料區（預設；`status: triaged`）——手動入庫通常已經確定要收，不用再回入庫建檔區按一次。
- 確認收＋標記閱讀中／超想看。
- 純事實小消息，直接標記送 PR（`status: ready`）。
- 先留在入庫建檔區，之後再篩（`status: inbox`，等於舊行為）。

另外可勾「存好後接著抓全文＋補 AI 閱讀建議與中文摘要」（預設開）：和 RSS 收件同一套，先抓可讀全文，再用選定引擎（預設隨機、失敗自動換）生成一句話推薦、三個閱讀理由與中文摘要；進度顯示在右下角狀態列，完成後直接帶到單篇材料頁。
