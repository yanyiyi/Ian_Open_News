...
Treat the local database as the editorial source of truth. Do not skip the local human decision layer: RSS candidates become inbox items, inbox items become either small-news PR material, featured-article skill candidates, or archived rejects.
...
3. Move only useful RSS candidates into `database/items.jsonl` as `status: inbox`.
4. Review inbox items locally.
   ...
   - Not useful: mark as `archived` with `local_decision.action: rejected` and a short reason.
